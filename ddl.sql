CREATE TABLE IF NOT EXISTS t_zip_record (
...
CREATE TABLE IF NOT EXISTS t_zip_task_status (
    id BIGSERIAL PRIMARY KEY,
    type VARCHAR(50) NOT NULL,
    ...
    CONSTRAINT ck_t_zip_task_status_status CHECK (status IN ('PENDING', 'SUCCESS', 'FAILED')),
    CONSTRAINT uq_t_zip_task_status_zip_path UNIQUE (zip_path)
);

CREATE INDEX IF NOT EXISTS idx_t_zip_task_status_updated_at_desc
ON t_zip_task_status (updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_t_zip_task_status_type
ON t_zip_task_status (type, updated_at DESC);

CREATE INDEX IF NOT EXISTS idx_t_zip_task_status_is_feedback
ON t_zip_task_status (is_feedback, updated_at DESC);

ALTER TABLE t_zip_task_status
ADD COLUMN IF NOT EXISTS is_feedback BOOLEAN NOT NULL DEFAULT FALSE;

CREATE TABLE IF NOT EXISTS t_map_path_config (
    id BIGSERIAL PRIMARY KEY,
...
ON t_map_path_config (enabled, sync_types);

CREATE TABLE IF NOT EXISTS t_service_lease (
...
ALTER TABLE t_service_lease
ADD COLUMN IF NOT EXISTS lease_token BIGINT NOT NULL DEFAULT 0;

CREATE TABLE IF NOT EXISTS t_zip_task_queue (
    id BIGSERIAL PRIMARY KEY,
    zip_path VARCHAR(1000) NOT NULL,
    status VARCHAR(20) NOT NULL,
...
ON t_zip_task_queue (status, next_retry_at, updated_at);

CREATE INDEX IF NOT EXISTS idx_t_zip_task_queue_locked_until
ON t_zip_task_queue (locked_until);
