...
- 运行：
  ...
  - `TASK_QUEUE_MAX_SIZE` / `EVENT_DEDUP_WINDOW_SEC` / `DASHBOARD_CACHE_TTL_SEC` / `DASHBOARD_AUTO_REFRESH_SEC`
  - `INITIAL_SCAN`
...
- `TASK_LOCK_RENEW_INTERVAL_SEC`：任务锁续约间隔（秒，必须小于 `TASK_LOCK_SEC`）
- `TASK_MAX_ATTEMPTS`：单任务最大尝试次数（超过后标记失败）
...
```json
{
  ...
  "DASHBOARD_AUTO_REFRESH_SEC": 5.0,
  "INITIAL_SCAN": true,
  "USE_PERSISTENT_QUEUE": true,
  "TASK_FETCH_BATCH_SIZE": 20,
  "TASK_LOCK_SEC": 120,
  "TASK_LOCK_RENEW_INTERVAL_SEC": 30,
  "TASK_MAX_ATTEMPTS": 20,
